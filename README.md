...
In JavaScript, it monitors for for an event and performs a specific action. .addEventListener() is applied to an element/object and a callback, event handler, performs the desired action.

4. Why would we convert a NodeList into an Array?
It's not required to do this but a perk of doing so is getting additional methods. NodeLists are array-like but don't feature many of the methods provided by the Array, like forEach, map, filter, etc. 
